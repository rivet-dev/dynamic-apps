...
- **`stdout` / `stderr`**: captured text written by the guest.
- **`exitCode`**: `0` on success, non-zero if the guest threw or was killed.
...
- **[Host Tools](/docs/features/host-tools)**: expose curated host functions to guest code.
