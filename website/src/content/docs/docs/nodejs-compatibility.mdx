...
- **Same runtime, full matrix**: the underlying VM is shared with agentOS, so the complete compatibility reference lives at [Node.js Compatibility on agentOS](https://agentos-sdk.dev/docs/node-compatibility).
