...
By default a `NodeRuntime` presents the full Node platform described above. The lower-level VM config (`CreateVmConfig.jsRuntime`) can change this: a non-`node` `platform` (`browser`, `neutral`, or `bare`) exposes no Node builtins, and under the `node` platform an explicit `allowedBuiltins` list narrows the set (an empty list denies all). This mirrors esbuild's `platform` vocabulary. Anything excluded becomes a denied builtin (`ERR_ACCESS_DENIED`).
