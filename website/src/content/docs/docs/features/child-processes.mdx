...
The handle exposes `pid`, `writeStdin` / `closeStdin`, `kill`, `wait`, and `exitCode`; see the [TypeScript SDK reference](/docs/sdks/typescript) for the full `NodeRuntimeProcess` and `NodeRuntimeSpawnOptions` shapes. Stream output by passing `onStdout` / `onStderr`, which receive raw `Uint8Array` chunks:
