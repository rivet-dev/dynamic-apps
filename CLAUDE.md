...
- **Wakeups are event-driven, never timer-polled.** A blocked guest `poll`/`net.poll_wait`/futex must be woken by an explicit readiness notification from whatever made it ready (socket data, pipe write, EOF, error, fd close), routed to *every* waiter that cares — including across processes (a pipe write notifies the **reader's** process, not just the writer's). Native Linux wakes an idle waiter 0 times until a real event; we must match that. **Never add a timer/poll fallback** (e.g. capping a block at 10ms to re-scan the fd set) to paper over a wakeup that the notify graph fails to deliver: that masks the real bug (an incomplete notify edge), burns a constant wakeup tax even when idle, and diverges from native. A missed wake is a permanent-hang bug — fix the missing notify edge, do not insure against it with churn. The *only* sanctioned periodic scan is an **opt-in, default-OFF debug "missed-wake" detector** that *flags/logs* a wake the event graph should have delivered (so you can find the missing edge), never one that silently *completes* the wait in production.
...
- **Linux behavior is the spec; any unlisted deviation is a bug.** The goal is to match a stock Linux box running the same binaries as closely as possible — same syscall semantics and `errno`, same `poll`/eventfd/pipe wake-and-drain behavior, same scheduler-visible idle (an app that drops to 0% CPU on Linux must go idle here too, not busy-spin). When the guest behaves differently it is a runtime fidelity gap to fix **here** (per the two bullets above), never the guest's fault. The only sanctioned exceptions:
  - **Single-threaded execution** *(intentional, permanent)* — no preemptive OS threads; `worker_threads`/wasi-threads + `SharedArrayBuffer` are opt-in and off by default. This is the one platform concession (see the bullet above).
  - **Bounded memory/CPU** *(intentional)* — Workers-style hard caps instead of Linux overcommit; OOM / CPU-limit kills are by design.
  - **Default-deny egress** *(intentional)* — the network is allowlist-gated, not an open host; unlisted destinations fail to connect by design.
  - **`mmap`** *(known gap, drive it down)* — memory-mapped file I/O is not faithfully supported; ported programs fall back to `read()`/buffered I/O (git and `uu_tac` are patched to do exactly this). A gap to close where feasible, **not** a license to feature-gate.

  Anything else that differs from Linux — `poll`/`revents`, eventfd/pipe drain, `CLOCK_MONOTONIC` advancing, signal delivery, fd inheritance, etc. — is a bug to drive down, not an exception to design around. Add a new exception here only for a genuine isolation property or third-party packaging quirk, with the reason.

- **A perf gap vs native Linux is a runtime bug until a 1:1 native trace proves otherwise — never call it a "floor."** When the *same binaries* are much slower (or otherwise diverge) under secure-exec than on a stock Linux box and the root cause isn't obvious, the FIRST question is **"why doesn't native have this?"** — not "what intrinsic cost explains it?" The same software is fast natively, so by "Linux behavior is the spec" the gap is a fidelity bug **here**, not an inherent property of the workload/toolkit/guest. Do not attribute it to the app (e.g. "GTK frame-clock pacing", "it's just how the toolkit redraws") without a native measurement that exhibits the *same* behavior. Diagnose it by building a **1:1 native-vs-secure-exec mapping**: run the identical binaries on native Linux measured the *same* way (a Docker Xvfb baseline works), and `strace`/`ltrace` both sides to compare syscall-for-syscall — poll timeouts, wake latency, round-trip counts, who blocks and for how long. The cautionary example: a ~50ms keystroke latency was nearly written off as an ~8ms-per-step "GTK frame-clock floor"; an `strace` of native mousepad on the *same* Xvfb showed the render is one ~2ms burst with `poll(fd,-1)` µs kernel wakes and **no such gaps** — proving it was a runtime `poll_wait` wake-latency bug (CORE-fixable), not a guest floor. If you ever catch yourself declaring a floor without having answered "why is native different here?", you have not found the root cause yet.
