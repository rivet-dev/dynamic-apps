/* fork()+exec -> posix_spawn DEFERRED transform for VTE (no real fork in wasm).
 * fork(): setjmp + return 0 (pretend child); the caller runs its child-setup (dup2/close), which we
 * RECORD into a posix_spawn_file_actions instead of applying; execve(): posix_spawn(recorded) + longjmp
 * back to fork() returning the child pid. Platform layer (constraint #5), VTE untouched. */
#include <setjmp.h>
#include <sys/types.h>
#include <unistd.h>
#include <errno.h>
...
pid_t fork(void) {
  ...
  if (setjmp(__fk_jmp) == 0)
    return 0;                 /* "child": caller does its setup + execve */
...
int execvpe(const char *file, char *const argv[], char *const envp[]) { return execve(file, argv, envp); }
/* dup2: record into file_actions if inside the deferred-fork child; else best-effort */
