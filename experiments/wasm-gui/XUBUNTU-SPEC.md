...
| **xfce4-settings** (xfsettingsd) | settings daemon → XSETTINGS push (theme/font/cursor) | https://gitlab.xfce.org/xfce/xfce4-settings | ✅ (XU1: GTK window themed Greybird via the XSETTINGS push. ALSO: the **xfce4-appearance-settings DIALOG** now renders (2026-06-26) -- the core Xubuntu "look" UI with real Style/Icons/Fonts/Settings tabs, Add/Help/Close, Greybird-themed, 0 fontconfig errors, all wasm, from the same UNMODIFIED source (scripts/build-appearance-settings.sh, standalone dialog via --disable-pluggable-dialogs). Proof gui-progress/2026-06-26T08/xu6-appearance-settings.png. KNOWN GAP: mounting /usr/share/themes so the Style tab ENUMERATES themes traps the guest (RuntimeError: unreachable at ~52s) -- the theme directory-scan/preview path, NOT CSS parsing (other apps apply Greybird's gtk.css fine); the dialog UI renders with an empty theme list. ALSO xfce4-ACCESSIBILITY-settings renders (2026-06-26): the Assistive Technologies / Keyboard / Mouse tabs, "Enable assistive technologies" toggle, Greybird-themed, real text, all wasm -- scripts/build-accessibility-settings.sh (same recipe; link only the dialog objects, NOT the sibling xfce4-find-cursor program that shares the dir). Proof gui-progress/2026-06-26T09/xu6-accessibility-settings.png. ALSO xfce4-DISPLAY-settings renders (2026-06-26): General/Advanced tabs + a live "screen" preview reading the REAL Xvfb display (Resolution 800x600 4:3) + Scale/Refresh/Rotation/Reflection dropdowns + Apply/Help/Close, all wasm -- queries the running display via xrandr (scripts/build-display-settings.sh; must also link the common/ objects, xfce_randr_new is in common/xfce-randr.c, + exo-2; colord disabled). Proof xu6-display-settings.png. ALSO xfce4-MOUSE-settings renders (2026-06-26): Devices/Behavior tabs, Device="Xvfb mouse" (reads the real device), Left/Right-handed radios + Reverse-scroll + Acceleration(2.0)/Sensitivity(4px) sliders + Reset/Help/Close, all wasm (scripts/build-mouse-settings.sh; +libxfce4util/xcursor/xi; libinput auto-disabled). No cursor-theme enumeration trap -- this config's tabs are Devices/Behavior, no Theme tab on construction. Proof xu6-mouse-settings.png) |
