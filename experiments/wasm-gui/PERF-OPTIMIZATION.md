...
| T-render5 | the 3ms `JAVASCRIPT_NET_POLL_MAX_WAIT` clamp makes a long-timeout poll_wait spin-wake every 3ms; harmless IF wakes are truly event-driven (notify), but if a real internal wake is MISSED the guest waits up to 3ms for the re-poll — possible per-gap tax up to ~3ms × ~7 | OPEN — check if the ~8ms internal wake is event-driven (immediate) or relies on the 3ms re-poll (then tightening/notify is a CORE lever) |
