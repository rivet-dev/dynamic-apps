...
| T-wake | **THE lever (native strace): wasm spreads the render over ~7 bursts × ~8ms; native does it in ONE ~2ms burst with `poll(-1)` µs kernel wakes. The ~8ms = data-arrival→guest-resume latency in the async poll_wait completion.** Eliminations: clamp 1/3/10ms → only ~2ms (NOT the clamp); `ASYNC_POLL=0` (inline-block) → WORSE ~110ms + fp 10s (async is load-bearing, keep it); not frame pacing (native one-burst), not the X server (replies in 60µs), not a pipe (pipetrace), not guest compute (~2.9× ≠ 80×) | OPEN — add a direct end-to-end probe: reader-thread data-arrival timestamp → guest poll_wait resume timestamp (SystemTime, same-process), confirm ~8ms, then find which handoff in `spawn_unix_socket_reader`→readiness.notify→poll_waiter_loop/deferred-completion→respond→guest owns it. HOPPROF wakeLag(notify→resume)=11µs and poll_wait d12=200µs do NOT account for ~8ms, so the fat segment is elsewhere (data→notify scheduling, or the guest doing N clamped re-polls per gap) |
