...
TP="$EXP/third_party"; PREFIX="$TP/wasm-prefix"; PCDIR="$PREFIX/lib/pkgconfig"
mkdir -p "$PREFIX/bin"
...
if [ ! -f "$PREFIX/lib/libgdk_pixbuf-2.0.a" ]; then
  ...
  cd gdk-pixbuf && rm -rf build-wasm
  meson setup build-wasm --cross-file "$CROSS_INI" --prefix="$PREFIX" -Dtests=false -Dman=false \
  ...
  ninja -C build-wasm gdk-pixbuf/libgdk_pixbuf-2.0.a >/dev/null 2>&1
  cp -f build-wasm/gdk-pixbuf/libgdk_pixbuf-2.0.a "$PREFIX/lib/"
  find build-wasm -name "gdk-pixbuf-2.0.pc" -exec cp -f {} "$PCDIR/" \;
  mkdir -p "$PREFIX/include/gdk-pixbuf-2.0/gdk-pixbuf"; cp -f gdk-pixbuf/*.h build-wasm/gdk-pixbuf/*.h "$PREFIX/include/gdk-pixbuf-2.0/gdk-pixbuf/" 2>/dev/null
fi
...
if [ ! -f "$PREFIX/lib/libatk-1.0.a" ]; then
  ...
  cd atk && rm -rf build-wasm
  meson setup build-wasm --cross-file "$CROSS_INI" --prefix="$PREFIX" -Dintrospection=false -Ddefault_library=static >/dev/null 2>&1
  ninja -C build-wasm atk/libatk-1.0.a >/dev/null 2>&1
  cp -f build-wasm/atk/libatk-1.0.a "$PREFIX/lib/"; find build-wasm -name "atk.pc" -exec cp -f {} "$PCDIR/" \;
  mkdir -p "$PREFIX/include/atk-1.0/atk"; cp -f atk/*.h build-wasm/atk/*.h "$PREFIX/include/atk-1.0/atk/" 2>/dev/null
fi
...
xbuild() { # name url dir liba
  [ -f "$PREFIX/lib/$4" ] && return
  cd "$TP"; [ -d "$3" ] || { curl -fsSL -o "$3.tar.xz" "$2" && tar xf "$3.tar.xz" && mv "$(basename "$2" .tar.xz)" "$3"; }
  cd "$TP/$3" && make distclean >/dev/null 2>&1
  CC="$CC" CFLAGS="$CFLAGS" LDFLAGS="-L$PREFIX/lib" AR="$AR" RANLIB="$WSDK/bin/llvm-ranlib" PKG_CONFIG_LIBDIR="$PCDIR" \
...
for t in glib-mkenums glib-genmarshal; do s=$(find "$TP/glib/build-wasm" -name "$t" -type f|head -1); [ -n "$s" ] && { cp -f "$s" "$PREFIX/bin/$t"; chmod +x "$PREFIX/bin/$t"; }; done
...
CFG=$(find "$TP/glib/build-wasm" -name config.py -path "*codegen*"|head -1); [ -n "$CFG" ] && cp -f "$CFG" "$TP/glib/gio/gdbus-2.0/codegen/config.py"
...
ninja -C build-wasm gtk/libgtk-3.a
echo "== M8: GTK 3.24 cross-compiles to wasm32-wasip1 -> $(stat -c%s build-wasm/gtk/libgtk-3.a) bytes =="