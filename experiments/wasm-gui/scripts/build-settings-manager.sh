#!/usr/bin/env bash
# XU6/XU1+: build the UNMODIFIED xfce4-settings 4.18.4 APPEARANCE dialog (xfce4-settings-manager) to wasm
...
[ -d "$SRC/xfce4-settings-manager" ] || { echo "FATAL: xfce4-settings not configured (run build-xfce4-settings.sh)"; exit 1; }
...
cd "$SRC"
rm -f xfce4-settings-manager/xfce4-settings-manager xfce4-settings-manager/*.o
...
make -k -j4 -C xfce4-settings-manager LDFLAGS="$LINK" >> /tmp/make-display.log 2>&1 || true
...
OBJS="$(ls "$SRC"/xfce4-settings-manager/xfce4_settings_manager-*.o "$SRC"/common/libxfce4_settings_la-*.o 2>/dev/null)"
...
GTKLIBS=$(PKG_CONFIG_LIBDIR="$PREFIX/lib/pkgconfig" pkg-config --static --libs gtk+-3.0 libxfce4ui-2 libxfconf-0 exo-2 garcon-1 libxfce4util-1.0 2>/dev/null)
BIN="$SRC/xfce4-settings-manager/xfce4-settings-manager"
...
echo "== fpcast-emu + -Oz -> xfce4-settings-manager.wasm =="
export PATH="/home/linuxbrew/.linuxbrew/bin:$PATH"
OUT="$EXP/xfce4-settings-manager.wasm"
...
echo "built xfce4-settings-manager.wasm ($(stat -c%s "$OUT") bytes)"
