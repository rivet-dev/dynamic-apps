...
PNG="${PNG:-$HOME/tmp/gui-progress/$(date -u +%Y-%m-%dT%H)/xu6-mousepad-typed.png}"; mkdir -p "$(dirname "$PNG")"
...
echo "running mousepad typing demo -> png=$PNG"
...
POST_INJECT_DELAY_MS=15000 timeout 240 env -u DISPLAY NO_AT_BRIDGE=1 "$HOST" --xdemo --timeout "${TIMEOUT:-170}" \
  ...
  --inject "h=button 1 400 300" --inject "h=focus" --inject "h=type $TEXT" \
