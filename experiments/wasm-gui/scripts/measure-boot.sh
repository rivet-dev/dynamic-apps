...
for run in $(seq 1 "$RUNS"); do
  ...
  docker run --rm \
    ...
    -e SX_SERIAL_SETTLE_MS="${SX_SERIAL_SETTLE_MS:-12000}" \
    -e SX_PARALLEL_VMS="${SX_PARALLEL_VMS:-0}" \
    -e SX_INFRA_DBUS_MS="${SX_INFRA_DBUS_MS:-2000}" \
    -e SX_INFRA_SVC_MS="${SX_INFRA_SVC_MS:-4000}" \
    -e SX_WM_FALLBACK_MS="${SX_WM_FALLBACK_MS:-12000}" \
    -e SX_LAUNCH_STAGGER_MS="${SX_LAUNCH_STAGGER_MS:-800}" \
    -e SX_BURST_LAUNCH="${SX_BURST_LAUNCH:-0}" \
    -e SX_READY_GATE="${SX_READY_GATE:-0}" \
    -e SECURE_EXEC_INLINE_SOCKET_DATA="${SECURE_EXEC_INLINE_SOCKET_DATA:-0}" \
    -e SECURE_EXEC_RPC_PROFILE="${SECURE_EXEC_RPC_PROFILE:-0}" \
    -e SECURE_EXEC_RPCPROF="${SECURE_EXEC_RPCPROF:-0}" \
    -e SECURE_EXEC_RPC_WATCHDOG_MS="${SECURE_EXEC_RPC_WATCHDOG_MS:-0}" \
    -e SECURE_EXEC_RPC_WATCHDOG_DUMP_MS="${SECURE_EXEC_RPC_WATCHDOG_DUMP_MS:-0}" \
    -e SECURE_EXEC_RPC_PROFILE_EVERY="${SECURE_EXEC_RPC_PROFILE_EVERY:-5000}" \
    ...
    -e SECURE_EXEC_PATHOPENPROF="${SECURE_EXEC_PATHOPENPROF:-0}" \
    -e SECURE_EXEC_RPC_BLOCK_US="${SECURE_EXEC_RPC_BLOCK_US:-300000}" \
    -e SECURE_EXEC_RECV_OFFBROKER="${SECURE_EXEC_RECV_OFFBROKER:-0}" \
    -e SECURE_EXEC_T1_RING="${SECURE_EXEC_T1_RING:-0}" \
    -e SECURE_EXEC_POLL_MAX_WAIT_MS="${SECURE_EXEC_POLL_MAX_WAIT_MS:-0}" \
    -e SECURE_EXEC_RTPROBE="${SECURE_EXEC_RTPROBE:-0}" \
    -e SECURE_EXEC_POLLSTAT="${SECURE_EXEC_POLLSTAT:-0}" \
    -e SECURE_EXEC_DEADLINE_PROBE="${SECURE_EXEC_DEADLINE_PROBE:-0}" \
    -e SECURE_EXEC_POLLWAITPROF="${SECURE_EXEC_POLLWAITPROF:-0}" \
    -e SECURE_EXEC_TEE_GUEST_STDERR="${SECURE_EXEC_TEE_GUEST_STDERR:-0}" \
    -e SECURE_EXEC_POLL_TRACE="${SECURE_EXEC_POLL_TRACE:-0}" \
    -e SECURE_EXEC_HOPPROF="${SECURE_EXEC_HOPPROF:-0}" \
    -v "$REPO:/repo" \
