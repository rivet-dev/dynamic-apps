...
# The HOST injects input by speaking X11+XTEST directly to the X server's host-backed AF_UNIX socket
# (--inject runs through the x11rb path in the host; no in-VM agent). This is exactly the channel the
# interactive --desktop window uses to forward real mouse/keyboard.
echo "== twm + input-target; host injects motion + ButtonPress via X11/XTEST =="
timeout 90 env -u DISPLAY "$HOST" --xdemo --timeout 25 \
  --server "$EXP/Xvfb.wasm" \
  --client "$EXP/twm.wasm" \
  --client "$EXP/guest-xclient/xinput-target.wasm" \
  --inject "host=motion 165 140" --inject "host=button 1 165 140" \
...
assert orange > 10000, "host XTEST ButtonPress was NOT delivered to the client (window not orange)"
assert blue < 5000, "client window still shows its initial colour (no input effect)"
print("PASS: host-driven X11/XTEST input reached a real libX11 client through the wasm X server")
PY
rm -f "$FB"
echo "== M6 input PASS =="
