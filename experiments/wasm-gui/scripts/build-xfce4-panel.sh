...
# -g0: drop DWARF from the panel's OWN objects (smaller .o). NOTE: this alone does NOT give a usable wasm
# name section for symbolized stackdumps -- the linked libs (libxfce4ui/libwnck/libgarcon/libgio) are
# built with -g, so their DWARF dominates the final binary and lld still omits the name section. A
# symbolized panel stackdump would need those libs rebuilt -g0 too (a deeper toolchain pass).
...
# EXTERNAL (out-of-tree) static plugins: "name:queried_symbol:actual_func:libname" (built separately, e.g.
# build-whiskermenu.sh -> libwhiskermenu.a). whiskermenu registers via xfce_panel_module_construct (the
# .desktop's X-XFCE-Module path), not xfce_panel_module_init -- the shim maps it to its construct func.
EXTERNAL_PLUGINS="${EXTERNAL_PLUGINS:-}"
PLUGIN_OBJS=""; EXT_LIBS=""
...
for spec in $EXTERNAL_PLUGINS; do
  ename="$(echo "$spec" | cut -d: -f1)"; efunc="$(echo "$spec" | cut -d: -f3)"; elib="$(echo "$spec" | cut -d: -f4)"
  echo "== external static plugin: $ename (func $efunc, lib lib$elib.a) =="
  echo "extern void *${efunc}(void);" >> "$GENC"
  EXT_LIBS="$EXT_LIBS -l$elib"
done
# External plugins may be C++ (whiskermenu) -> the panel link (C linker) needs the C++ stdlib for
# operator new/delete (_Znwm etc.) and the C++ ABI/unwind.
[ -n "$EXTERNAL_PLUGINS" ] && EXT_LIBS="$EXT_LIBS -lc++ -lc++abi"
{
  ...
  for spec in $EXTERNAL_PLUGINS; do
    ename="$(echo "$spec" | cut -d: -f1)"; esym="$(echo "$spec" | cut -d: -f2)"; efunc="$(echo "$spec" | cut -d: -f3)"
    echo "  if (strcmp(name, \"$ename\") == 0 && strcmp(symbol, \"$esym\") == 0) return (void *) ${efunc};"
  done
  echo "  return 0;"
  echo "}"
} >> "$GENC"
...
GTKTRANS="$RESO $WNCKRESO $SHIMOBJS $EXT_LIBS -lxfce4ui-2 -lgarcon-gtk3-1 -lgarcon-1 -lexo-2 -lwnck-3 -lxfconf-0 -lXinerama -latk-bridge-2.0 -latk-1.0 -lepoxy -lXi -lXrandr -lXcursor -lXcomposite -lXdamage -lXfixes -lXtst -lXft -lXrender -lXext -lX11 -lXau -lXdmcp"
LINK="$LINK0 $WRAP"
