#!/usr/bin/env bash
# XU6/XU1+: build the UNMODIFIED xfce4-settings 4.18.4 APPEARANCE dialog (xfce4-mouse-settings) to wasm
...
[ -d "$SRC/dialogs/mouse-settings" ] || { echo "FATAL: xfce4-settings not configured (run build-xfce4-settings.sh)"; exit 1; }
...
cd "$SRC"
rm -f dialogs/mouse-settings/xfce4-mouse-settings dialogs/mouse-settings/*.o
echo "== compile common/ + the mouse dialog objects (libtool binary discarded) =="
make -k -j4 -C common LDFLAGS="$LINK" >> /tmp/make-mouse.log 2>&1 || true
make -k -j4 -C dialogs/mouse-settings LDFLAGS="$LINK" >> /tmp/make-mouse.log 2>&1 || true
# mouse-settings links the dialog's objects PLUS the common/ lib objects -- xfce_randr_new() lives in
...
OBJS="$(ls "$SRC"/dialogs/mouse-settings/xfce4_mouse_settings-*.o "$SRC"/common/libxfce4_settings_la-*.o 2>/dev/null)"
echo "dialog objects: $(echo $OBJS | wc -w)"
[ -n "$OBJS" ] || { echo "no objects built"; tail -25 /tmp/make-mouse.log; exit 1; }
...
GTKLIBS=$(PKG_CONFIG_LIBDIR="$PREFIX/lib/pkgconfig" pkg-config --static --libs gtk+-3.0 libxfce4ui-2 libxfconf-0 exo-2 libxfce4util-1.0 xcursor xi 2>/dev/null)
BIN="$SRC/dialogs/mouse-settings/xfce4-mouse-settings"
...
"$WSDK/bin/clang" $LDFLAGS -lhostcompat $OBJS -o "$BIN" \
  ...
  "$SETJMP" -Wl,--allow-undefined $WRAPS -Wl,-z,stack-size=8388608 2>> /tmp/make-mouse.log
RC=$?; echo "mouse-settings link rc=$RC"
if [ ! -f "$BIN" ]; then echo "no binary"; grep -aiE "error|undefined symbol" /tmp/make-mouse.log | head -12; exit 1; fi
echo "binary $(stat -c%s "$BIN") bytes; __wasi_init_tp: $("$WSDK/bin/llvm-nm" "$BIN" 2>/dev/null | grep -acawE '__wasi_init_tp')"

echo "== fpcast-emu + -Oz -> xfce4-mouse-settings.wasm =="
export PATH="/home/linuxbrew/.linuxbrew/bin:$PATH"
OUT="$EXP/xfce4-mouse-settings.wasm"
...
echo "built xfce4-mouse-settings.wasm ($(stat -c%s "$OUT") bytes)"
