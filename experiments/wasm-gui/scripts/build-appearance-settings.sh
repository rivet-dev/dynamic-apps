#!/usr/bin/env bash
# XU6/XU1+: build the UNMODIFIED xfce4-settings 4.18.4 APPEARANCE dialog (xfce4-appearance-settings) to wasm
...
[ -d "$SRC/dialogs/appearance-settings" ] || { echo "FATAL: xfce4-settings not configured (run build-xfce4-settings.sh)"; exit 1; }
...
cd "$SRC"
rm -f dialogs/appearance-settings/xfce4-appearance-settings dialogs/appearance-settings/*.o
echo "== compile common/ + the appearance dialog objects (libtool binary discarded) =="
make -k -j4 -C common LDFLAGS="$LINK" >> /tmp/make-appearance.log 2>&1 || true
make -k -j4 -C dialogs/appearance-settings LDFLAGS="$LINK" >> /tmp/make-appearance.log 2>&1 || true
OBJS="$(ls "$SRC"/dialogs/appearance-settings/*.o 2>/dev/null)"
echo "dialog objects: $(echo $OBJS | wc -w)"
[ -n "$OBJS" ] || { echo "no objects built"; tail -25 /tmp/make-appearance.log; exit 1; }
...
BIN="$SRC/dialogs/appearance-settings/xfce4-appearance-settings"
...
"$WSDK/bin/clang" $LDFLAGS -lhostcompat $OBJS -o "$BIN" \
  ...
  "$SETJMP" -Wl,--allow-undefined $WRAPS -Wl,-z,stack-size=8388608 2>> /tmp/make-appearance.log
RC=$?; echo "appearance-settings link rc=$RC"
if [ ! -f "$BIN" ]; then echo "no binary"; grep -aiE "error|undefined symbol" /tmp/make-appearance.log | head -12; exit 1; fi
echo "binary $(stat -c%s "$BIN") bytes; __wasi_init_tp: $("$WSDK/bin/llvm-nm" "$BIN" 2>/dev/null | grep -acawE '__wasi_init_tp')"

echo "== fpcast-emu + -Oz -> xfce4-appearance-settings.wasm =="
export PATH="/home/linuxbrew/.linuxbrew/bin:$PATH"
OUT="$EXP/xfce4-appearance-settings.wasm"
...
echo "built xfce4-appearance-settings.wasm ($(stat -c%s "$OUT") bytes)"
