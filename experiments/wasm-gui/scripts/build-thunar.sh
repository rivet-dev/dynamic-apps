...
# gmodule static-plugin support via the SHARED toolchain/gmodule-shim.c (the SAME mechanism the xfce4-panel
# plugins use): --wrap=g_module_* returns a name-carrying handle for a plugin .so path and resolves the entry
# points from a generated static table (panel_static_plugin_lookup). For thunar there is one thunarx provider
# (thunar-sbr) with 3 entry points. The table's references to those symbols also PULL the SBR from its archive
# and keep it past --gc-sections, so no separate keep-shim/symwrap/sentinel is needed. See GMODULE-STATIC-PLUGINS.md.
GMODSHIM="$EXP/toolchain/gmodule-shim.o"
"$WSDK/bin/clang" --target=wasm32-wasip1-threads --sysroot="$WSDK/share/wasi-sysroot" -O2 -pthread \
  -c "$EXP/toolchain/gmodule-shim.c" -o "$GMODSHIM"
GENC="$EXP/toolchain/gmodule-plugins-thunar.gen.c"
{
  echo "/* GENERATED by build-thunar.sh -- static thunarx-plugin registry for the shared gmodule shim. */"
  echo "#include <string.h>"
  echo "extern void thunar_extension_initialize(void *plugin);"
  echo "extern void thunar_extension_shutdown(void);"
  echo "extern void thunar_extension_list_types(const void *types, int *n_types);"
  echo "void *panel_static_plugin_lookup(const char *name, const char *symbol) {"
  echo "  if (strcmp(name, \"thunar-sbr\") == 0 && strcmp(symbol, \"thunar_extension_initialize\") == 0) return (void *) thunar_extension_initialize;"
  echo "  if (strcmp(name, \"thunar-sbr\") == 0 && strcmp(symbol, \"thunar_extension_shutdown\")   == 0) return (void *) thunar_extension_shutdown;"
  echo "  if (strcmp(name, \"thunar-sbr\") == 0 && strcmp(symbol, \"thunar_extension_list_types\") == 0) return (void *) thunar_extension_list_types;"
  echo "  return 0;"
  echo "}"
} > "$GENC"
GMODTABLE="$EXP/toolchain/gmodule-plugins-thunar.gen.o"
"$WSDK/bin/clang" --target=wasm32-wasip1-threads --sysroot="$WSDK/share/wasi-sysroot" -O2 -pthread -c "$GENC" -o "$GMODTABLE"
...
GTKTRANS="$RESO $WNCKRESO $VFSSHIM $EMPTYSHIM $GMODSHIM $GMODTABLE -lxfce4ui-2 -lexo-2 -lwnck-3 -lexif -lXinerama -latk-bridge-2.0 -latk-1.0 -lepoxy -lXi -lXrandr -lXcursor -lXcomposite -lXdamage -lXfixes -lXtst -lXft -lXrender -lXext -lX11 -lXau -lXdmcp"
GMODWRAP="-Wl,--wrap=g_module_open -Wl,--wrap=g_module_open_full -Wl,--wrap=g_module_symbol -Wl,--wrap=g_module_close -Wl,--wrap=g_module_make_resident -Wl,--wrap=g_module_error -Wl,--wrap=g_module_supported"
LINK="-L$PREFIX/lib -lglibcompat $LDFLAGS -ldbuscreds -Wl,--allow-undefined -Wl,--wrap=read -Wl,--wrap=getsockopt -Wl,--wrap=writev -Wl,--wrap=g_vfs_get_default -Wl,--wrap=open -Wl,--wrap=openat -Wl,--wrap=fopen -Wl,--wrap=stat -Wl,--wrap=lstat $GMODWRAP -Wl,-z,stack-size=8388608"
