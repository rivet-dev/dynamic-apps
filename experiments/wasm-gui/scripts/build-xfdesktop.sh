...
# FILE-VIEW GATE fix: wrap g_vfs_get_default -> g_vfs_get_local. The sandbox has no dlopen (no gvfs daemon
# backends), so the default GVfs is always the local vfs; this returns it directly + bypasses the trapping
# _g_io_modules_ensure_loaded module-init machinery, unblocking xfdesktop's desktop-icon g_file_query_info
# enumeration. Toolchain --wrap shim (constraint #5); see toolchain/gio-vfs-local-shim.c.
