...
APP="$EXP/xfce4-appfinder.wasm"; W=800; H=600
PNG="${PNG:-$HOME/tmp/gui-progress/$(date -u +%Y-%m-%dT%H)/xu6-appfinder-search.png}"; mkdir -p "$(dirname "$PNG")"
...
bash scripts/prepare-rootmenu-fixtures.sh /tmp/vmrootmenu >/dev/null 2>&1
FB="$(mktemp /tmp/type-fb.XXXXXX.bin)"
TEXT="te"
echo "running appfinder search demo -> png=$PNG"
...
POST_INJECT_DELAY_MS=15000 timeout 240 env -u DISPLAY NO_AT_BRIDGE=1 "$HOST" --xdemo --timeout "${TIMEOUT:-170}" \
  ...
  --vm-tree /tmp/vmxu5sess --vm-tree /tmp/vmicons --vm-tree /tmp/vmxft --vm-tree /tmp/vmschemas --vm-tree /tmp/vmxkb --vm-tree /tmp/vmrootmenu \
  --inject "h=type $TEXT" \
